...
class appDevUrlMatcher extends Symfony\Bundle\FrameworkBundle\Routing\RedirectableUrlMatcher
{
    ...
    public function match($pathinfo)
    {
        ...
        if (0 === strpos($pathinfo, '/add')) {
            ...
            // replays
            if (0 === strpos($pathinfo, '/addreplay') && preg_match('#^/addreplay/(?P<comment>[^/]++)/(?P<article>[^/]++)$#s', $pathinfo, $matches)) {
                if (!in_array($this->context->getMethod(), array('GET', 'POST', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'POST', 'HEAD'));
                    goto not_replays;
                }

                return $this->mergeDefaults(array_replace($matches, array('_route' => 'replays')), array (  '_controller' => 'frontBundle\\Controller\\DefaultController::addreplayAction',));
            }
            not_replays:
